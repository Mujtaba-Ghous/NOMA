...
% % % % % % % %     Transmit beamvectors (PEV) G-RQ (2-users only)
...
% % % % % % % %     Transmit beamvectors (PEV) G-ZF (2-users only)
    ...
    %%%% % % % % % % %     Transmit beamvectors (PEV) G-MF (2-users only)
    wk_MF=[];
...
%%%%%%%%%%%%%% Simulation Setup FOR MF  %%%%%%%%%%%%%%%

[A, B, Y, CDF] = Coovariance_Model_Sim(wk_MF, M, N, K, var_v, Phi_k,Phi_F,Phi_NF,PS,PS_dB,alpha,rho,RthF,naN ,ncN,pN,pF,ncF,naF,eta,NoumberOfTrial,xbins);
...
%%%%%%%%%%%%%% Analytical FOR MF   %%%%%%%%%%%%%%%%%%%%%
  ...
  CDFA_MF =   Coovariance_Model_Analy(wk_MF, M, N, K, var_v, Phi_k,PS,PS_dB,alpha,rho,RthF,pF,pN, tol);
    
   CDF_Anal_MF = CDFA_MF;

    CDF_ZF =   Coovariance_Model_Analy(wk_ZF, M, N, K, var_v, Phi_k,PS,PS_dB,alpha,rho,RthF,pF,pN, tol);
    
   CDF_SIM_ZF = CDF_ZF;

    CDFA_ZF =   Coovariance_Model_Analy(wk_ZF, M, N, K, var_v, Phi_k,PS,PS_dB,alpha,rho,RthF,pF,pN, tol);
    
   CDF_Anal_ZF = CDFA_ZF;

%%%%%%%%%%%%%% Simulation Setup FOR Rayleigh Quotient G-RQ


[A, B, Y, CDF_RQ] = Coovariance_Model_Sim(wk_RQ, M, N, K, var_v, Phi_k,Phi_F,Phi_NF,PS,PS_dB,alpha,rho,RthF,naN ,ncN,pN,pF,ncF,naF,eta,NoumberOfTrial,xbins);

CDF_SIM_RQ = CDF_RQ;

%%%%%%%%%%%%%% Analytical FOR Rayleigh Quotient G-RQ

CDFA_RQ =   Coovariance_Model_Analy(wk_RQ, M, N, K, var_v, Phi_k,PS,PS_dB,alpha,rho,RthF,pF,pN, tol);
...
plot(PS_dB,CDF_SIM_RQ,'o');  % Simulation result of OP using Rayleigh Quotient
plot(PS_dB,CDF_Anal_RQ,'-'); % Analytical results of OP using Rayleugh Quotient

plot(PS_dB,CDF_SIM_ZF,'*');
plot(PS_dB,CDF_Anal_ZF,'-');


plot(PS_dB,CDF_SIM_MF,'<');
plot(PS_dB,CDF_Anal_MF,'-');

ylim([0 1])

legend('G-RQ (sim.) M_B=2', 'G-RQ (ana.) M_B=2', 'G-ZF (sim.) M_B=2','G-ZF (ana.) M_B=2','G-MRT (sim.) M_B=2','G-MRT (ana.) M_B=2',...
    'Location', 'Best');
xlabel('SNR in dB', 'Interpreter', 'Latex');
ylabel('Outage probability of user-F','Interpreter', 'Latex');
