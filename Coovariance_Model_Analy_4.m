function[CDFA] = Coovariance_Model_Analy_4(wk, M, N, K, var_v, Phi_k,PS,g2,alpha,rho,RthF,pF,pN, tol)
...
%         keyboard
           ...
           for ss = 1:length(g2)
    count_2 = 0;
   % disp(strcat('SNR=',num2str(g2(ss)),'dB'));
...
ncF     = (10^(-6))*1e6;
             c  = eta*PS*(2*alpha(aa)/(1-alpha(aa))+rho(rr))/(naF + ncF);
                        ...
                        P= A1.*pF.*PS  - g2(ss)*B1.*pN.*PS;      %(2^(y(kk))-1)
                         P1= B1.*pF.*PS  - g2(ss)*A1.*pN.*PS;      %(2^(y(kk))-1)
                        Pmat{k} =P;
%                         [Up,Lamp]=eig(P);
% 
%                         lamp=(diag(Lamp));
...
%                         keyboard
                      ...
                      Sum1(n) = ((lamp(n))^T /abs(lamp(n))*(1/Prod(n))*exp(-g2(ss)*(naF+ncF)/lamp(n))*heaviside(g2(ss)*(naF+ncF)/lamp(n)));
                      Sum2(n) = ((lamp_1(n))^T /abs(lamp_1(n))*(1/Prod_1(n))*exp(-g2(ss)*((1-rho).*naN+ncN)/lamp_1(n))*heaviside(g2(ss)*((1-rho).*naN+ncN)/lamp_1(n)));
                     ...
                     extpara = lSN.*trace(A1);
                      ...
                      FA(ss)= heaviside(g2(ss)*(naF+ncF))-Sum;  
                       

                      FA_1(ss)= heaviside(g2(ss)*((1-rho).*naN+ncN))-Sum_1; 
                       ...
                       int(ss) = integral(@(t) exp(-t-g2(ss)/(extpara*lNF*c*t)),mu_a/extpara,1,'ArrayValued',true)
