%Simulation Setup
function[A, B, Y, CDF] = Coovariance_Model_Sim_4(wk, M, N, K, var_v, Phi_k,Phi_F,Phi_NF,PS,g2,alpha,rho,RthF,naN ,ncN,pN,pF,ncF,naF,eta, NoumberOfTrial,xbins)
Y = [];
A = [];
B = [];
CDF = [];
...
%        Z_NF=rBF*hhNF(runs,:)*diag(Lamc)*(rNF*hhN(runs,:))'; 
       ...
       SINR_N_sF = (1-rho(rr)).*pF.*PS.*(h_N*A1*h_N')./ ((1-rho(rr)).*pN.*PS.*real(h_N*B1*h_N') + (1-rho(rr))*naN + ncN);
        SINR_F_sF =pF.*PS.*(h_F*A1*h_F')./(pN.*PS.*(h_F*B1*h_F') + naF + ncF );
        SINR_NF = eta.*PS.*(h_N*A1*h_N').*h_NF.* (2*alpha(aa)/(1-alpha(aa))+rho(rr))/(naF + ncF);
   
 
 if SINR_N_sF >= g2(ss) && SINR_F_sF< g2(ss)
     count_2 = count_2 + 1;
     
 elseif (SINR_N_sF < g2(ss)) && (SINR_F_sF < g2(ss))
     count_2 = count_2 + 1;
 end
      end
